...
if(!file.exists("data/Survey_results_final.csv") ||
  ...
  err_str <- paste("You're missing data/figure files. Download them and put them into the folder with this Rmd file:", getwd())
  stop(err_str)
}
```
...
This tutorial is designed to get you up and running with R as quickly as possible. If you have any questions, come to office hours (listed on Canvas), or ask questions on [Piazza](piazza.com/upenn/spring2021/stat571stat701).

In order to run this RMarkdown file, you will need to have the following .csv files in the *data/* folder
and the .png file in the *fig/* folder. Both *data/* and *fig/* folders should be in the same directory with the .Rmd file:
...
It is always to good idea to keep your project folder organized as follows.
...
You can reorganize these panels in *View -> Panes*. More details on changing the location of the panes and customizing Rstudio further can be [here](https://support.rstudio.com/hc/en-us/articles/200549016-Customizing-RStudio).

## Conda

The other way to install R and RStudio is via Anaconda. See *R_installation_Guide* and [the official documentation](https://docs.anaconda.com/anaconda/user-guide/tasks/using-r-language/).
...
One R project is recommended for one project (e.g. lecture, homework, final project). Great things about R projects:
...
* **RData**: You can reload all variables/data stored in the *environment* from last session. Specifically,
...
* **Rhistory**: You can reload all the code you have run in previous sessions. Similar to RData, when you exit close a project, RStudio will save all history commands into .Rhistory. You can browser the history commands in the *History* tab or by Up/Down in the *Console*. 
...
# Step 2: Creating a Rmarkdown document
...
Three backticks indicate that the block is a code block, and `{r}` indicates that the code is in the R language. You can choose to show/hide codes by specifying `echo=TRUE`/`echo=FALSE`, and to show nothing but only run the codes by `include=FALSE`. [More options](https://yihui.name/knitr/options/) are available. You can also click on the setting (the little gear button at the topright corner of each chunk) to check options. 

**It is a good practice to name your r chunk: `{r name}`**. 
This will be very helpful to locate your bug when debugging.
Note that duplicated r chunk name is not allowed.
...
You can create a vector and operate on them. Notice that '#' is for you to write comment (similar to Python or '//' for C/C++).
```{r}
a <- c(1,2,3)
b <- c(1,1,1)
...
We can also generate a vector of sequence of number using `seq()`.
Inside `()`, we set function parameter by `argument = parameter`.
```{r}
...
Throughout the course, we will run a lots of simulations so we need to generate random numbers from various distributions. 
...
```{r}
# load functions in src/functions.R 
...
### `for` loop
`for` loop is one of the most basic building blocks of most programming languages.
...
### `while` loop
Another way to run repeatedly is using `while`. The basic syntax of a `while` loop in r is
```{r}
cat("
while (condition)
{
...
It means *while* we are under *condition*, run the statement. Similar, here is an example
...
Indeed, R is slow for *loops*. R is a functional programming languages, which means it is 
...
we can directly input the vector to the function directly.

```{r}
inputs <- seq(1, 10, 2)
inputs^2 # square(inputs)
```
We will encounter this very often. Rule of thumb is that if you can
vectorize, do NOT use loop. Here are some cases when loops make sense.

1. Using a function that don't take vector arguments
...
duplicated(x, fromLast = F) # find out the duplicated, use fromLast to consider duplication from last
```
...
You can write data out as different format, e.g. `.csv`, `.txt`, using `write.csv()` and `write.table()`. You can also save an R object (variables that you assign using `<-`) as an .rds file using `saveRDS()` and restore it using `readRDS()`. 


```{r eval=F}
write.csv(tips, "output/tips.csv") # write the tips data as tips.csv

saveRDS(model, "output/linear_regression.rds") # save the lm object into linear_greassion.rds
readRDS("output/linear_regression.rds") # read in linear_greassion.rds, i.e. the model object
```
...
For this course's final project, we will be requiring your group to uploaded it to Github using Git. You are encouraged, but not required, to use it for the homework assignments as well.
...
* An excellent reference for the powerful and easy to use graphics package ggplot2 for more complex graphics: [link]https://rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf)
